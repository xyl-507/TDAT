...
from ltr.models.fastervit.faster_vit_xyl import FasterViT
...
class DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, classification_layer, bb_regressor_layer):
        ...
        self.fastervit = FasterViT(dim=1024, ct_size=1, window_size=2, resolution=22)  # for test_feat_clf  ct_size=1, window_size=2
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        train_feat = self.extract_backbone_features(train_imgs.reshape(-1, *train_imgs.shape[-3:]))  # train_imgs=(3,8,3,352,352) --> (24,3,352,352), tracking阶段是 (1,3,352,352)
        test_feat = self.extract_backbone_features(test_imgs.reshape(-1, *test_imgs.shape[-3:]))  # test_imgs=(3,8,3,352,352)

        # Classification features
        train_feat_clf = self.get_backbone_clf_feat(train_feat)  # train_feat: layer2=(24,512,44,44), layer3=(24,1024,22,22), 用的是layer3
        test_feat_clf = self.get_backbone_clf_feat(test_feat)

        # fastervit xyl20240306
        test_feat_clf = self.fastervit(test_feat_clf)
