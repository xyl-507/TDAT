...
if __name__ == '__main__':
    ...
    global_tokenizer = TokenInitializer(dim,
                                        input_resolution,
                                        window_size,
                                        ct_size=ct_size)
    do_gt = True
    ...
    input = torch.randn(24, dim, input_resolution, input_resolution)  # w、h和 input_resolution一致
    print('input:', input.shape)

    ct = global_tokenizer(input) if do_gt else None
    B, C, H, W = input.shape
    if transformer_block:
        input = window_partition(input, window_size)
    output, ct = m(input, ct)
    if transformer_block:
        output = window_reverse(output, window_size, H, W, B)
