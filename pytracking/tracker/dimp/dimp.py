...
# from ltr.models.polyper.polyper_xyl import RefineAttention
...
class DiMP(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, self.get_centered_sample_pos(),
                                                                                  ...
                                                                                  self.img_sample_sz)
        # -------------------fastervit + Extract classification features  xyl20240306-----------------------------------
        with torch.no_grad():
            backbone_feat_no_hat = self.net.get_backbone_clf_feat(backbone_feat)
            backbone_feat_hat = self.net.fastervit(backbone_feat_no_hat)  # backbone_feat_hat 1,1024,22,22
            test_x = self.net.classifier.extract_classification_feat(backbone_feat_hat)  # test_x 1,512,22,22

        if self.visdom is not None:
            self.visdom.register(backbone_feat_no_hat.mean(dim=1).squeeze(), 'heatmap', 1, 'backbone_feat_no_hat')
            self.visdom.register(backbone_feat_hat.mean(dim=1).squeeze(), 'heatmap', 1, 'backbone_feat_hat')
            self.visdom.register(test_x.mean(dim=1).squeeze(), 'heatmap', 1, 'test_x_hat')
        # -------------------fastervit + Extract classification features  xyl20240306-----------------------------------

        # Extract classification features
        test_x = self.get_classification_features(backbone_feat)  # test_x=(1,512,22,22)

        if self.visdom is not None:
            test_x_no_hat = self.get_classification_features(backbone_feat)  # test_x=(1,512,22,22)
            scores_raw_no_hat = self.classify_target(test_x_no_hat)
            sample_pos, sample_scales = self.get_sample_location(sample_coords)
            _, scale_ind_no_hat, s_no_hat, flag_no_hat = self.localize_target(scores_raw_no_hat, sample_pos, sample_scales)
            score_maps_no_hat = s_no_hat[scale_ind_no_hat, ...]  # score_map=(23,23)
            self.visdom.register(score_maps_no_hat, 'heatmap', 2, 'Score Map_no_hat' + self.id_str)
            self.visdom.register(test_x_no_hat.mean(dim=1).squeeze(), 'heatmap', 1, 'test_x_no_hat')
            # self.visdom.register(test_x.mean(dim=1).squeeze(), 'heatmap', 1, 'test_x')

        # Location of sample
        sample_pos, sample_scales = self.get_sample_location(sample_coords)

        # Compute classification scores
        scores_raw = self.classify_target(test_x)  # scores_raw=(1,1,23,23)

        # Localize the target
        translation_vec, scale_ind, s, flag = self.localize_target(scores_raw, sample_pos, sample_scales)  # s=(1,23,23)
        ...
        score_map = s[scale_ind, ...]  # score_map=(23,23)
