...
class UAVTrack112Dataset(BaseDataset):
    ...
    def __init__(self):
        ...
        self.base_path = '/home/xyl/newdrive/0.test_datasets/UAVTrack112'  # 更改数据集时需要修改的地方
    ...
    def _construct_sequence(self, sequence_info):
        ...
        anno_path = '{}-anno/anno/{}'.format(self.base_path, sequence_info['anno_path'])  # 更改数据集时需要修改的地方
        # NOTE: OTB has some weird annos which panda cannot handle
        ground_truth_rect = load_text(str(anno_path), delimiter=(',', None), dtype=np.float64, backend='numpy')

        return Sequence(sequence_info['name'], frames, 'uavtrack112', ground_truth_rect[init_omit:, :],  # 更改数据集时需要修改的地方
    ...
    def _get_sequence_info_list(self):
        ...
        for root, dirs, files in os.walk('/home/xyl/newdrive/0.test_datasets/UAVTrack112'):  # 更改数据集时需要修改的地方
            for dir in dirs:
        ...
        for i in range(len(sequence_dir)):
            ...
            sequence_info["anno_path"] = os.path.join(sequence_info["name"]+'.txt')  # 更改数据集时需要修改的地方，标注文件为 air conditioning box1.txt
