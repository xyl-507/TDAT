...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        redetection = False  # xyl 20240120  扩大搜索区域
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            try:
                if tracker.params.redetection:  # 开启扩大搜索区域 xyl20240120
                    out, flag = tracker.track(image, info, redetection)  # xyl 20240120  扩大搜索区域
                    if flag in ['not_found', 'uncertain', 'hard_negative']:  # xyl 20240120  扩大搜索区域
                        redetection = True
                        # print('{} global redetection'.format(frame_num+1))
                    else:
                        redetection = False
                else:
