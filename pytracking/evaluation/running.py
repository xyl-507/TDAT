...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name)  # 分数据集来生成结果 xyl20240308
...
def _save_tracker_output_votlt(seq: Sequence, tracker: Tracker, output: dict):  # 生成VOTLT格式的结果，xyl20240120
    """Saves the output of the tracker."""

    base_results_path = os.path.join(tracker.results_dir, 'longterm', seq.name)

    if not os.path.exists(base_results_path):
        os.makedirs(base_results_path)
    base_results_path = os.path.join(tracker.results_dir, 'longterm', seq.name, seq.name)
    segmentation_path = os.path.join(tracker.segmentation_dir, seq.name)
    ...
    for key, data in output.items():
        ...
        if key == 'target_bbox':
            ...
            if isinstance(data[0], (dict, OrderedDict)):
                ...
                # scores_file = '{}_object_presence_scores.txt'.format(base_results_path)
                scores_file = '{}_001_confidence.value'.format(base_results_path)
                save_scores(scores_file, data)
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, visdom_info=None):
    ...
    def _results_exist():
        ...
        elif seq.dataset == 'vot2018lt' or seq.dataset == 'vot2019lt':  # 生成VOTLT格式的结果，xyl20240120
            bbox_file = os.path.join(tracker.results_dir, 'longterm', seq.name, '{}_001.txt'.format(seq.name))
            return os.path.isfile(bbox_file)
        elif seq.object_ids is None:
            # bbox_file = '{}/{}.txt'.format(tracker.results_dir, seq.name)
            bbox_file = '{}/{}/{}.txt'.format(tracker.results_dir, seq.dataset, seq.name)  # 分数据集来生成结果 xyl20240308
            return os.path.isfile(bbox_file)
        else:
    ...
    if not debug:
        if seq.dataset == 'oxuva':
            ...
            _save_tracker_output_votlt(seq, tracker, output)
        else:
...
def run_dataset(dataset, trackers, debug=False, threads=0, visdom_info=None):
    ...
    if mode == 'sequential':
        # for seq in dataset:
        for num, seq in enumerate(dataset):
            print('---------------------- {}/{} ----------------------'.format(num+1, len(dataset)))  # xyl20240504，打印序列的序号
